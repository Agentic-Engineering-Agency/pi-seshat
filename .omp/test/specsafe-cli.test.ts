...
import { readStateFileOrNull, statePathFor, type CostCounter, type StateFile } from "../hooks/specsafe-session";

const cliPath = path.resolve(process.cwd(), ".omp/skills/specsafe/bin/specsafe.ts");
...
describe("[unit] specsafe CLI lifecycle", () => {
	...
	test("C6 CLI-written state parses through readStateFileOrNull and matches the known-good begin snapshot", () => {
		...
		const rawState = JSON.parse(fs.readFileSync(statePathFor(tempDir), "utf-8")) as StateFile;
	...
	test("C9 corrupt state is quarantined and begin treats it as empty state", () => {
		...
		expect(entries.some((entry) => /^\.honcho-state\.json\.corrupt-\d+$/.test(entry))).toBe(true);
		const state = readState();
		expect(state?.currentSlice?.id).toBe("TEST-001");
		expect(state?.history).toEqual([]);
	});
