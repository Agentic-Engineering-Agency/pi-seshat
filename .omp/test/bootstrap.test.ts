 ...
 *
 * The production CLI is expected at `.omp/skills/bootstrap/bin/bootstrap.ts`.
 * These tests run it from hermetic temporary fixture projects and must never
 * write to the real pi-seshat `.pi/` tree.
 */
...
const bootstrapBin = path.resolve(process.cwd(), ".omp/skills/bootstrap/bin/bootstrap.ts");
const piSeshatRoot = process.cwd();
const expectedOmpTarget = path.join(piSeshatRoot, ".omp");
...
function runBootstrap(args: string[] = [], cwd = tempDir): SpawnSyncReturns<string> {
...
function statMtimes(paths: string[]): Record<string, number> {
	...
	for (const relativePath of paths) {
		mtimes[relativePath] = fs.lstatSync(path.join(tempDir, relativePath)).mtimeMs;
	}
	return mtimes;
}
...
function auditLogLines(): string[] {
	return fs.readFileSync(auditLogPath(), "utf-8").trim().split("\n").filter(Boolean);
}
...
function dryRunPreviewIsEmpty(stdout: string): boolean {
	const actionLines = stdout.split(/\r?\n/).filter((line) => /\bwould\s+/.test(line));
...
describe("[unit] bootstrap skill", () => {
	...
	test("C7 apply writes a 0600 JSONL audit log with bootstrap action and luci approver", () => {
		...
		const entry = JSON.parse(lines[0]) as { action?: unknown; approver?: unknown };
	...
	test("C8 re-apply only appends audit history and a later dry-run leaves the bootstrapped project unchanged", () => {
		...
		const beforeDryRunMtimes = statMtimes([...stablePaths, ".gitignore", ".pi/.bootstrap-log.jsonl"]);
		const dryRun = runBootstrap();
		expect(dryRun.status).toBe(0);
		expect(dryRunPreviewIsEmpty(dryRun.stdout)).toBe(true);
		expect(statMtimes([...stablePaths, ".gitignore", ".pi/.bootstrap-log.jsonl"])).toEqual(beforeDryRunMtimes);
	});
	...
	test("C10 apply refuses a regular .omp directory conflict and --force-symlink replaces it with the correct symlink", () => {
		...
		expect(fs.lstatSync(path.join(tempDir, ".omp")).isSymbolicLink()).toBe(false);

		const forced = applyBootstrap("--force-symlink");
		expect(forced.status).toBe(0);
		expect(fs.lstatSync(path.join(tempDir, ".omp")).isSymbolicLink()).toBe(true);
