/**
 * Black-box tests for .omp/skills/env-doctor/bin/env-doctor.ts.
 *
...
const cliPath = path.resolve(process.cwd(), ".omp/skills/env-doctor/bin/env-doctor.ts");
...
function createTestEnv(overrides: Partial<Record<StubName, StubSpec>> = {}): TestEnv {
	...
	for (const name of ["hooks", "tools", "agents", "skills"] as const) {
		...
		fs.symlinkSync(path.join(cwd, ".omp", name), path.join(home, ".omp", "agent", name));
	}
	...
	const stubs: Record<StubName, string> = {
		honcho: makeExecutableStub(path.join(stubsDir, "honcho-probe"), specs.honcho),
		linear: makeExecutableStub(path.join(stubsDir, "linear-probe"), specs.linear),
...
function defaultDoctorEnv(testEnv: TestEnv, extraEnv: Record<string, string> = {}): Record<string, string> {
	return {
		...cleanProcessEnv(),
		HOME: testEnv.home,
...
describe("[unit] env-doctor CLI", () => {
	...
	test("T-f failing omp config stub exits 1 and reports omp config FAIL", () => {
		const env = createTestEnv({ omp: { exitCode: 1, stderr: "missing config" } });
	...
	test("T-h valid .pi/.honcho-state.json passes the state-file check", () => {
		const env = createTestEnv();
		writeHonchoState(env.cwd, JSON.stringify({ currentSlice: null, history: [] }));
	...
	test("T-h agent honcho config passes when present, 0600, and parseable", () => {
		const env = createTestEnv();
		writeAgentHonchoConfig(env.home, JSON.stringify({ workspaceId: "ws-test" }));
	...
	test("T-i honcho probe 401-style failure exits 1 and reports HONCHO_API_KEY FAIL", () => {
		const env = createTestEnv({ honcho: { exitCode: 1, stderr: "401 unauthorized" } });
	...
	test("T-k API key values are redacted from stdout and stderr", () => {
		const env = createTestEnv({ honcho: { exitCode: 1, stderr: "bad key hnc_secrettoken_zzz rejected" } });

		const result = runDoctor(env, [], { HONCHO_API_KEY: "hnc_secrettoken_zzz" });
		const output = combinedOutput(result);
