...
const COHERENCE_TS = path.resolve(process.cwd(), ".omp/skills/coherence/bin/coherence.ts");
...
function baseEnv(repo: string, overrides: Record<string, string | undefined> = {}): NodeJS.ProcessEnv {
	const env: NodeJS.ProcessEnv = {
		...
		PI_COHERENCE_LINEAR_LIST_FIXTURE: path.join(LINEAR_FIXTURE_DIR, "list-clean.txt"),
...
function withoutLinearApiKey(repo: string, overrides: Record<string, string | undefined> = {}): NodeJS.ProcessEnv {
...
function runCoherenceWithoutLinearKey(repo: string, args: string[]): SpawnSyncReturns<string> {
	return spawnSync("bun", ["run", COHERENCE_TS, ...args], {
		cwd: repo,
		encoding: "utf-8",
...
function expectOneStdoutLine(result: SpawnSyncReturns<string>, pattern: RegExp): void {
...
function createCommitHistory(repo: string): void {
	...
	execGit(repo, ["commit", "--allow-empty", "-m", "fix: login\n\nSpec-Slice: CUR-92"]);
	execGit(repo, ["commit", "--allow-empty", "-m", "chore: still no trailer"]);
	execGit(repo, ["commit", "--allow-empty", "-m", "test: another slice\n\nSpec-Slice: CUR-92"]);
...
describe("[unit] coherence skill", () => {
	...
	test("C3 linear-vs-specs reports orphan-spec when spec exists but Linear has no matching ticket", () => {
		...
		const result = runCoherence(repo, ["check", "linear-vs-specs"], {
			PI_COHERENCE_LINEAR_LIST_FIXTURE: path.join(LINEAR_FIXTURE_DIR, "list-empty.txt"),
		});
	...
	test("C4 trailers-vs-linear parses commits with and without Spec-Slice trailers", () => {
		...
		const result = runCoherence(repo, ["check", "trailers-vs-linear", "--range=HEAD~5..HEAD"]);
	...
	test("C5 trailers-vs-linear reports stale-trailer when trailer ticket is still in triage", () => {
		...
		expect(result.status).toBe(1);
		expect(stdoutLines(result).some((line) => /^\[stale-trailer\]/.test(line))).toBe(true);
	});
