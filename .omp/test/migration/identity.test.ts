...
const LIVE = process.env.OMP_LIVE_TESTS === "1"
...
describe("[integration] §4.7 — validator identity attribution under real omp dispatch", () => {
	...
	test.skip(
		"[OMP_LIVE_TESTS=1 required] validator conclude is attributed to validator, not parent HONCHO_PEER_ID",
		async () => {
			if (!LIVE) {
			...
			try {
				...
				//   await proc.exited;
				//
				throw new Error("TODO: implementer must wire real omp task dispatch here");
