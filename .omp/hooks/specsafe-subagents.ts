 ...
 */
function summarizeForCommit(content: ReadonlyArray<{ type: string; text?: string }>): string {
	...
	pi.on("tool_result", async (event, ctx) => {
		...
		try {
			const result = commitSubagentWork({
				...
				message: summarizeForCommit(event.content as ReadonlyArray<{ type: string; text?: string }>),
			});
			if (result.committed && ctx.hasUI) {
				ctx.ui.notify(`auto-commit: ${agentName} on slice ${slice.id}`, "info");
			}
			if (result.error) {
				pi.logger.warn(`[specsafe-subagents] auto-commit error: ${result.error}`);
			}
		} catch (err) {
			pi.logger.error(
