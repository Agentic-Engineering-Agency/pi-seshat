...
function cmdBegin(args: string[], statePath: string): number {
	...
	if (!sliceId || !workspaceId || !sessionId) {
		fail(`error: begin requires <slice-id> <workspace-id> <session-id>\n${USAGE}`, 2);
	}

	const state = loadState(statePath);
	if (state.currentSlice) {
		fail(`error: slice already open (${state.currentSlice.id}); end it before beginning a new one`, 1);
	}
...
function cmdEnd(args: string[], statePath: string): number {
	...
	if (outcomeRaw !== "PASS" && outcomeRaw !== "FAIL" && outcomeRaw !== "ABANDONED") {
		fail(`error: outcome must be one of PASS|FAIL|ABANDONED (got: ${outcomeRaw})\n${USAGE}`, 2);
	}
	...
	const entry: HistoryEntry = {
		...
		outcome,
		costSummary: { ...slice.costCounter, subagentTokens: { ...slice.costCounter.subagentTokens } },
	};
