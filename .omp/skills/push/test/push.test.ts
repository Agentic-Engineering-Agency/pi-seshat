...
describe("[unit] AC4 — mid-slice: currentSlice != null is rejected", () => {
	test("exits non-zero, message mentions slice still open", async () => {
		...
		expect(result.exitCode).not.toBe(0);
		const combined = result.stdout + result.stderr;
		expect(combined.toLowerCase()).toMatch(/slice still open|specsafe_end|current.?slice/i);
	});
	...
	test("honcho-state.json with currentSlice=null passes check", async () => {
		...
		const stateFile = path.join(env.repo, ".pi", ".honcho-state.json");
		fs.writeFileSync(stateFile, JSON.stringify({ currentSlice: null, history: [] }));
...
describe("[unit] pre-flight: dirty working tree fails", () => {
	test("exits non-zero when tree is dirty", async () => {
		...
		expect(result.exitCode).not.toBe(0);
		const combined = result.stdout + result.stderr;
		expect(combined.toLowerCase()).toMatch(/dirty|clean|uncommitted|working tree/i);
	});
});
