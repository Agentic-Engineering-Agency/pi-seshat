...
const REGISTRY_PATH = path.join(cwd, ".pi", "skills", "latest-docs", "registry.json");
const LATEST_DOCS = path.join(cwd, ".omp", "skills", "latest-docs", "bin", "latest-docs.ts");
...
function loadRegistry(): Record<string, unknown> {
	...
	try {
		return JSON.parse(fs.readFileSync(REGISTRY_PATH, "utf8")) as Record<string, unknown>;
	} catch (err) {
...
function probeOne(lib: string): number {
	...
	if (res.error) {
		console.error(`failed to spawn latest-docs for ${lib}: ${res.error.message}`);
		process.exit(1);
	}
...
function main(): void {
	...
	for (const [key, val] of Object.entries(registry)) {
		...
		// probe succeeded.
		const after: RegistryEntry = success ? { ...before, verified: true } : before;
