...
function sanitize(text: string, secrets: ReadonlyArray<string | undefined>): string {
	let out = text;
...
function parseStateFile(filePath: string): ParseResult {
	...
	try {
		...
		if (!("currentSlice" in parsed) || !Array.isArray((parsed as { history?: unknown }).history)) {
	...
	try {
		...
		try {
			...
			if (perm !== 0o600 && process.platform !== "win32") {
				return { kind: "fail", reason: `mode ${perm.toString(8)} (expected 0600)` };
			}
		} catch {
...
function checkLinear(env: NodeJS.ProcessEnv, strict: boolean, secrets: string[]): Item {
...
function parseArgs(argv: string[]): { strict: boolean; json: boolean } | { error: string } {
	let strict = false;
	let json = false;
	for (const a of argv) {
...
function main(): number {
	...
	const anyFail = (Object.keys(results) as Key[]).some((k) => results[k].status === "FAIL");
