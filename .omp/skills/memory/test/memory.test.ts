		...
		history: [
			{
				...
				costSummary: {
					honchoCalls: 10,
					honchoCost: 0.002,
					subagentTokens: { input: 5000, output: 1000, cacheRead: 0, cacheWrite: 0, cost: 0.015, turns: 3 },
				},
			},
			{
				...
				costSummary: {
					honchoCalls: 5,
					honchoCost: 0.001,
					subagentTokens: { input: 2000, output: 500, cacheRead: 100, cacheWrite: 50, cost: 0.005, turns: 2 },
				},
			},
		],
	};
...
function neverCall(name: string) {
	return mock((..._args: unknown[]) => {
		throw new Error(`WRITE METHOD CALLED: ${name} — this is forbidden in the memory skill`);
	});
}
	...
	function makePeer(peerId: string) {
		...
		return {
			conclusions: {
				...
				async list(_opts?: { page?: number; size?: number }): Promise<AsyncIterable<MockConclusion>> {
					async function* gen() {
...
describe("[unit] status — open slice", () => {
	test("[AC5] prints slice id, session id, workspace id, and counter numbers", async () => {
		...
		const result = await dispatch(["status"], { cwd: tmpDir, env: baseEnv, honchoClientFactory: factory });
...
describe("[unit] cost — finished slice from history", () => {
	...
	test("uses current slice when no sliceId given", async () => {
		...
		writeState(tmpDir, makeOpenSliceState());
		const { factory } = makeMockFactory({});
		const result = await dispatch(["cost"], { cwd: tmpDir, env: baseEnv, honchoClientFactory: factory });
		expect(result.exit).toBe(0);
		expect(result.stdout).toContain("SPEC-20260424-003");
	});
...
describe("[unit] review — time window filter", () => {
	...
	const mockConclusions: Record<string, MockConclusion[]> = {
		validator: [
			{ id: "c-inside1", content: "conclusion inside window", created_at: inside1.toISOString() },
			{ id: "c-inside2", content: "conclusion near end", created_at: inside2.toISOString() },
			{ id: "c-grace-begin", content: "conclusion within begin grace", created_at: justBeforeGrace.toISOString() },
			{ id: "c-grace-end", content: "conclusion within end grace", created_at: justAfterGrace.toISOString() },
			{ id: "c-too-early", content: "conclusion too early - EXCLUDED", created_at: tooEarly.toISOString() },
			{ id: "c-too-late", content: "conclusion too late - EXCLUDED", created_at: tooLate.toISOString() },
		],
		reviewer: [],
		steward: [],
	};
	...
	test("[AC7] calls conclusions.list on each expected peer (validator, reviewer, steward)", async () => {
		...
		const trackingFactory = mock((_env: Record<string, string | undefined>) => ({
	...
	test("exits 1 when no session-id argument given", async () => {
		...
		const result = await dispatch(["review"], { cwd: tmpDir, env: baseEnv, honchoClientFactory: factory });
...
describe("[unit] [AC8] no write methods called by any command", () => {
	test("none of status, cost, history, review, search call write methods", async () => {
		...
		for (const [[cmd, ...args]] of commands) {
			await dispatch([cmd, ...args], { cwd: tmpDir, env: baseEnv, honchoClientFactory: trackFactory });
		}
...
describe("[unit] history", () => {
	test("prints all history entries by default (up to 10)", async () => {
		...
		const result = await dispatch(["history"], { cwd: tmpDir, env: baseEnv, honchoClientFactory: factory });
		expect(result.exit).toBe(0);
...
describe("[unit] search", () => {
	test("returns formatted hits from mock", async () => {
		const tmpDir = makeTempDir();
		writeState(tmpDir, makeOpenSliceState());
		const { factory } = makeMockFactory({ searchHits: [{ content: "a result from honcho" }, { content: "another one" }] });
	...
	test("exits 1 when no query argument", async () => {
		...
		const result = await dispatch(["search"], { cwd: tmpDir, env: baseEnv, honchoClientFactory: factory });
		expect(result.exit).toBe(1);
		expect(result.stderr.toLowerCase()).toContain("usage");
	});
...
describe("[unit] unknown command", () => {
	test("exits 1 with usage message", async () => {
		const tmpDir = makeTempDir();
		const { factory } = makeMockFactory({});
		const result = await dispatch(["bogus"], { cwd: tmpDir, env: baseEnv, honchoClientFactory: factory });
		expect(result.exit).toBe(1);
		expect(result.stderr.toLowerCase()).toContain("usage");
	});
});
