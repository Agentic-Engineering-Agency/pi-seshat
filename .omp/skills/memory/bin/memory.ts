...
export type HonchoClient = {
	peer(peerId: string): Promise<{
		conclusions: {
			...
			list(opts?: { page?: number; size?: number }): Promise<AsyncIterable<MockConclusion>>;
		};
...
export type HonchoClientFactory = (env: Record<string, string | undefined>) => HonchoClient;
...
function findSessionInState(state: StateFile, sessionId: string): { entry: CurrentSlice | HistoryEntry; endedAt: string } | null {
	// Check currentSlice
...
async function cmdStatus(state: StateFile): Promise<DispatchResult> {
	...
	if (!state.currentSlice) {
		...
		lines.push(`${padLabel("subagent_turns:")}${padNum(cc.subagentTokens.turns)}`);
		lines.push(`${padLabel("subagent_input_tok:")}${padNum(cc.subagentTokens.input)}`);
		lines.push(`${padLabel("subagent_output_tok:")}${padNum(cc.subagentTokens.output)}`);
	}
...
async function cmdReview(
	...
	if (!workspaceId) {
		return {
			stdout: "",
			stderr: "error: HONCHO_WORKSPACE_ID is required (set env var or ensure state file has workspaceId)",
			exit: 2,
		};
	}

	const client = honchoClientFactory({ ...env, HONCHO_WORKSPACE_ID: workspaceId });
...
async function cmdCost(sliceId: string | undefined, state: StateFile): Promise<DispatchResult> {
	let id: string;
	let cc: CostCounter;
	...
	if (!sliceId) {
		...
		if (!state.currentSlice) {
			...
			if (!last) {
				return { stdout: "", stderr: "error: no slice currently open and no history entries", exit: 1 };
			}
			id = last.sliceId;
			cc = last.costSummary;
		} else {
...
async function cmdHistory(limit: number, state: StateFile): Promise<DispatchResult> {
...
async function cmdSearch(
	...
	const workspaceId =
		...
		(state.history.length > 0 ? state.history[state.history.length - 1]?.workspaceId : undefined);

	if (!workspaceId) {
		return {
			stdout: "",
			stderr: "error: HONCHO_WORKSPACE_ID required (set env var or open/have a slice in history)",
			exit: 2,
		};
	}
	...
	try {
		...
		const raw = err instanceof Error ? err.message : String(err);
		return { stdout: "", stderr: `error: honcho search failed: ${raw}`, exit: 2 };
	}
  ...
  search <query>           search Honcho at workspace scope`;
	...
	switch (command) {
		...
		case "review": {
			...
			if (!sessionId) {
				return { stdout: "", stderr: `usage: memory review <session-id>\n\n${USAGE}`, exit: 1 };
			}
		...
		case "search": {
			...
			if (!query) {
				return { stdout: "", stderr: `usage: memory search <query>\n\n${USAGE}`, exit: 1 };
			}
...
function realHonchoClientFactory(env: Record<string, string | undefined>): HonchoClient {
