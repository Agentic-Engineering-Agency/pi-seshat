...
function listPendingDrafts(cwd: string): Array<{ id: string; filePath: string; content: string; header: PatchHeader }> {
...
function findDraft(cwd: string, id: string): { filePath: string; content: string; header: PatchHeader } | null {
...
function runGitApplyFile(patchContent: string, cwd: string, git: DispatchOpts["gitRunner"]): GitRunnerResult {
...
async function cmdPropose(
	...
	// Validate diff
	if (!diffBody.trim()) {
		return { stdout: "", stderr: "invalid unified diff: stdin was empty", exit: 1 };
	}
...
async function cmdList(opts: DispatchOpts): Promise<DispatchResult> {
	...
	const rows = drafts.map((d) => {
		const rat = d.header.rationale.length > 60 ? `${d.header.rationale.slice(0, 60)}...` : d.header.rationale;
		return [
			d.id.padEnd(ID_W),
...
async function cmdShow(id: string, opts: DispatchOpts): Promise<DispatchResult> {
...
async function cmdApply(id: string, approve: boolean, opts: DispatchOpts): Promise<DispatchResult> {
	const draft = findDraft(opts.cwd, id);
	if (!draft) {
		return { stdout: "", stderr: `no draft found with id: ${id}`, exit: 1 };
	}
	...
	if (statusResult.exit !== 0 || statusResult.stdout.trim() !== "") {
		return { stdout: "", stderr: "working tree must be clean before docs apply", exit: 1 };
	}
...
async function cmdDiscard(id: string, opts: DispatchOpts): Promise<DispatchResult> {
	const draft = findDraft(opts.cwd, id);
	if (!draft) {
		return { stdout: "", stderr: `no draft found with id: ${id}`, exit: 1 };
	}
...
export async function dispatch(argv: string[], opts: DispatchOpts): Promise<DispatchResult> {
	...
	switch (command) {
		case "propose": {
			...
			if (!filePath) {
				return { stdout: "", stderr: `usage: docs propose <path> --rationale=<r>\n\n${USAGE}`, exit: 1 };
			}
			let rationale = "";
			...
			if (!rationale) {
				return { stdout: "", stderr: `--rationale is required\n\n${USAGE}`, exit: 1 };
			}
		...
		case "show": {
			const id = rest[0];
			if (!id) {
				return { stdout: "", stderr: `usage: docs show <id>\n\n${USAGE}`, exit: 1 };
			}
		...
		case "apply": {
			const id = rest[0];
			if (!id) {
				return { stdout: "", stderr: `usage: docs apply <id> [--i-approve]\n\n${USAGE}`, exit: 1 };
			}
		...
		case "discard": {
			const id = rest[0];
			if (!id) {
				return { stdout: "", stderr: `usage: docs discard <id>\n\n${USAGE}`, exit: 1 };
			}
