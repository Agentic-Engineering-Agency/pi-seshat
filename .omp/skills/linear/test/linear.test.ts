...
function mockIssue(opts: {
  ...
  return {
    ...
    state: async () => ({ id: opts.stateId, name: opts.stateName, type: opts.stateType }),
    assignee: async () => opts.assigneeId ? mockUser(opts.assigneeId, opts.assigneeName ?? "Luci") : null,
...
function makeMockClient(overrides?: Partial<LinearClientLike>): LinearClientLike {
...
function makeOpts(
  ...
  return {
    ...
    linearClientFactory: clientFactory ? (_key: string) => clientFactory() : (_key: string) => makeMockClient(),
    now: nowFn,
  };
}
...
describe("[unit] linear comment", () => {
  ...
  test("[unit] comment --i-approve calls createComment exactly once and appends to log", async () => {
    ...
    const opts = makeOpts(tmpDir, () => mockClient, () => now);
    const result = await dispatch(["comment", "CUR-92", "hi", "--i-approve"], opts);
...
describe("[unit] linear transition", () => {
  ...
  test("[unit] transition --i-approve calls updateIssue with resolved stateId", async () => {
    const now = new Date("2026-04-24T12:00:00Z");
    const opts = makeOpts(tmpDir, () => mockClient, () => now);
    const result = await dispatch(["transition", "CUR-92", "in_review", "--i-approve"], opts);

    expect(result.exit).toBe(0);
    expect(mockClient.updateIssue).toHaveBeenCalledTimes(1);
    const [id, input] = (mockClient.updateIssue as ReturnType<typeof mock>).mock.calls[0] as [string, { stateId: string }];
  ...
  test("[unit] transition to unknown state name returns exit 1", async () => {
    const opts = makeOpts(tmpDir, () => mockClient);
    const result = await dispatch(["transition", "CUR-92", "nonexistent_state"], opts);
    expect(result.exit).toBe(1);
...
describe("[unit] linear create", () => {
  ...
  test("[unit] create without --i-approve prints preview payload", async () => {
    ...
    const result = await dispatch(
      ["create", "--team=team-uuid-1", "--title=New Feature", "--body=Some description"],
      opts,
    );
...
describe("[live] linear smoke tests", () => {
  ...
  test.skipIf(!LIVE)("[live] linear list returns at least one issue", async () => {
    ...
    try {
      const opts: DispatchOpts = {
        ...
        linearClientFactory: (key: string) => new LinearClient({ apiKey: key }) as unknown as LinearClientLike,
      };
      const result = await dispatch(["list"], opts);
      expect(result.exit).toBe(0);
