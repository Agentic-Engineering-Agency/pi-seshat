...
export type IssueNode = {
  ...
  state: () => Promise<{ id: string; name: string; type: string } | null | undefined>;
...
export type LinearClientLike = {
  ...
  createIssue(input: unknown): Promise<{ success: boolean; issue?: { id: string; identifier: string } }>;
  createComment(input: unknown): Promise<{ success: boolean; comment?: { id: string } }>;
...
async function cmdComment(
  ...
  if (!key || !body) {
    return { stdout: "", stderr: "usage: linear comment <KEY> <body> [--i-approve]", exit: 1 };
  }
  ...
  if (!result.success) {
    return { stdout: "", stderr: "error: createComment returned success=false", exit: 2 };
  }
...
async function cmdTransition(
  ...
  if (!key || !stateToken) {
    return { stdout: "", stderr: "usage: linear transition <KEY> <state> [--i-approve]", exit: 1 };
  }
  ...
  if (!teamInfo?.id) {
    return { stdout: "", stderr: `error: could not determine team for issue ${key}`, exit: 2 };
  }
  ...
  if (!result.success) {
    return { stdout: "", stderr: "error: updateIssue returned success=false", exit: 2 };
  }
  ...
  const logEntry = {
    ...
    action: "transition",
    key,
    before: { state: currentState?.name ?? null, stateId: issue.stateId ?? null },
...
async function cmdCreate(
  ...
  if (!result.success) {
    return { stdout: "", stderr: "error: createIssue returned success=false", exit: 2 };
  }
...
function wrapLinearClient(sdkClient: import("@linear/sdk").LinearClient): LinearClientLike {
  return {
    async issues(variables?: unknown) {
      const conn = await sdkClient.issues(variables as import("@linear/sdk").IssuesQueryVariables);
      return {
    ...
    async createIssue(input: unknown) {
      const payload = await sdkClient.createIssue(input as import("@linear/sdk").IssueCreateInput);
      const issue = await payload.issue;
      return {
        success: payload.success,
        issue: issue ? { id: issue.id, identifier: issue.identifier } : undefined,
      };
    },

    async createComment(input: unknown) {
      const payload = await sdkClient.createComment(input as import("@linear/sdk").CommentCreateInput);
    ...
    async updateIssue(id: string, input: unknown) {
      const payload = await sdkClient.updateIssue(id, input as import("@linear/sdk").IssueUpdateInput);
    ...
    async team(id: string) {
      ...
      return {
        ...
        states: async () => {
          ...
          return {
            nodes: conn.nodes.map((s) => ({ id: s.id, name: s.name, type: s.type })),
          };
        },
      };
    },
