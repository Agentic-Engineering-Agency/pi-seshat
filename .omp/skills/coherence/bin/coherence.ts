...
function runLinear(subArgs: string[], cwd: string): LinearResult {
	...
	if (stub) {
		...
		cmd = "bun";
		args = ["run", path.resolve(process.cwd(), ".omp/skills/linear/bin/linear.ts"), ...subArgs];
	}
...
function parseLinearList(stdout: string): Ticket[] {
	...
	for (const raw of lines) {
		...
		for (let i = 0; i < headerCols.length; i++) {
			...
			const end = i + 1 < headerCols.length ? headerCols[i + 1]!.start : line.length;
...
function briefMatches(briefFiles: string[], key: string): boolean {
	...
	for (const f of briefFiles) {
		...
		if (f.startsWith(`${key}-`) || f.startsWith(`${key}__`) || f.startsWith(`${key}_`)) {
			return true;
		}
	}
	return false;
...
function emitLinearUnavailable(): number {
	process.stderr.write("Linear integration unavailable: LINEAR_API_KEY not set\n");
	return 2;
}
...
function cmdTrailersVsLinear(repo: string, range: string): number {
	...
	if (g.status !== 0) {
		process.stderr.write(`coherence: git log failed: ${(g.stderr ?? "").trim()}\n`);
		return 2;
	}
	...
	for (const key of trailerKeys) {
		...
		if (got.status !== 0) {
			drift.push(`[orphan-trailer] ${key} commit references unknown Linear ticket`);
			continue;
		}
		const state = parseLinearGetState(got.stdout);
		if (STALE_STATES.has(state)) {
			drift.push(`[stale-trailer] ${key} commit trailer points to ticket in ${state}`);
		}
	}
...
function cmdBriefCoverage(repo: string): number {
	...
	const open = parseLinearList(r.stdout).filter((t) => OPEN_STATES.has(t.state));
	const briefFiles = listBriefFiles(repo);

	const drift: string[] = [];
	for (const t of open) {
		if (!briefMatches(briefFiles, t.key)) {
			drift.push(`[orphan-brief] ${t.key} ${t.state}, no brief in specs/briefs/`);
		}
	}
