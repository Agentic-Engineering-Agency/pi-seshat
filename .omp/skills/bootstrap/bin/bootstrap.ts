...
// bin/bootstrap.ts → .omp/skills/bootstrap/bin → repo root is 4 levels up.
const PI_SESHAT_ROOT = path.resolve(__dirname, "..", "..", "..", "..");
const OMP_SOURCE = path.join(PI_SESHAT_ROOT, ".omp");
...
function parseArgs(argv: string[]): Opts {
	...
	for (const arg of argv) {
		...
		else if (arg.startsWith("--workspace=")) opts.workspace = arg.slice("--workspace=".length);
...
function computePlan(cwd: string, opts: Opts): Plan {
	...
	const gitignoreMissing = GITIGNORE_PATTERNS.filter((p) => !existingLines.has(p));
	...
	return { dirsMissing, symlinkAction, agentsExists, claudeExists, gitignoreMissing, hasMutations };
}

function detectPiSeshatSelf(cwd: string): boolean {
	if (path.resolve(cwd) === PI_SESHAT_ROOT) return true;
	// Defense-in-depth: a regular .omp directory holding a real bootstrap skill
	// means we're inside (a clone of) pi-seshat itself.
	const ompPath = path.join(cwd, ".omp");
	const ompStat = safeLstat(ompPath);
	if (!ompStat || ompStat.isSymbolicLink() || !ompStat.isDirectory()) return false;
	const sentinel = path.join(cwd, ".omp", "skills", "bootstrap", "SKILL.md");
...
function previewMode(cwd: string, plan: Plan, out: NodeJS.WriteStream): number {
	...
	if (plan.symlinkAction === "create") {
		...
		out.write(`would symlink .omp → ${OMP_SOURCE} (replacing existing directory)\n`);
	}
	...
	if (plan.gitignoreMissing.length > 0) {
		out.write(`would update .gitignore (${plan.gitignoreMissing.length} patterns)\n`);
	}
	out.write("would write audit log entry\n");
	return 0;
}

function applyMode(cwd: string, plan: Plan, opts: Opts, out: NodeJS.WriteStream): number {
	...
	const honchoWorkspace = opts.workspace ?? process.env.HONCHO_WORKSPACE_ID ?? "default";
	...
	if (plan.gitignoreMissing.length > 0) {
		...
		out.write(`updated .gitignore (${plan.gitignoreMissing.length} patterns)\n`);
...
function main(argv: string[]): number {
	...
	if (detectPiSeshatSelf(cwd)) {
		fail("error: Refusing to bootstrap pi-seshat self: cwd is the source repo", 1);
	}
