...
function makeStubGitRunner(
	results: Record<string, { stdout: string; stderr: string; exit: number }> = {},
): {
...
function makeBaseOpts(tmpDir: string, overrides: Partial<DispatchOpts> = {}): DispatchOpts {
...
function writeHonchoState(tmpDir: string, sliceId: string | null): void {
	...
	fs.writeFileSync(path.join(piDir, ".honcho-state.json"), JSON.stringify(state, null, 2), { mode: 0o600 });
}
...
describe("[unit] AC11 — scope validation", () => {
	...
	test("docs/PRD.md is accepted (no scope error)", async () => {
		...
		const result = await dispatch(["propose", "docs/PRD.md", "--rationale=add X section"], opts);
	...
	test("specs/brief.md is accepted", async () => {
		...
		const result = await dispatch(["propose", "specs/brief.md", "--rationale=test"], opts);
		expect(result.stderr).not.toContain("scoped to BMad artifacts");
		expect(result.exit).toBe(0);
	});
	...
	test("specs/briefs/CUR-92.md is accepted", async () => {
		...
		const result = await dispatch(["propose", "specs/briefs/CUR-92.md", "--rationale=test"], opts);
		expect(result.stderr).not.toContain("scoped to BMad artifacts");
		expect(result.exit).toBe(0);
	});
	...
	test("src/foo.ts is rejected with scope message", async () => {
		const opts = makeBaseOpts(tmpDir, { stdin: makeStdin(VALID_DIFF) });
		const result = await dispatch(["propose", "src/foo.ts", "--rationale=hack"], opts);
	...
	test("README.md is rejected (not in allowlist)", async () => {
		const opts = makeBaseOpts(tmpDir, { stdin: makeStdin(VALID_DIFF) });
		const result = await dispatch(["propose", "README.md", "--rationale=x"], opts);
		expect(result.exit).not.toBe(0);
		expect(result.stderr).toContain("scoped to BMad artifacts");
	});
	...
	test("docs-extra/foo.md is rejected (not docs/, specs/, specs/briefs/)", async () => {
		const opts = makeBaseOpts(tmpDir, { stdin: makeStdin(VALID_DIFF) });
		const result = await dispatch(["propose", "docs-extra/foo.md", "--rationale=x"], opts);
		expect(result.exit).not.toBe(0);
		expect(result.stderr).toContain("scoped to BMad artifacts");
	});
});
...
describe("[unit] diff validation", () => {
	...
	test("invalid diff (no --- +++ headers) is rejected at propose time", async () => {
		const opts = makeBaseOpts(tmpDir, { stdin: makeStdin("this is not a unified diff\njust some text\n") });
		const result = await dispatch(["propose", "docs/PRD.md", "--rationale=test"], opts);
		expect(result.exit).not.toBe(0);
		expect(result.stderr.toLowerCase()).toMatch(/invalid|not a valid|unified diff/i);
	});
...
describe("[unit] AC8 — propose creates .patch file", () => {
	...
	test("creates patch under .pi/.doc-drafts/, has mode 0600, appears in list", async () => {
		...
		// Propose
		const proposeResult = await dispatch(["propose", "docs/PRD.md", "--rationale=add X section"], opts);
...
describe("[unit] docs show", () => {
	...
	test("show with nonexistent id exits non-zero", async () => {
		const result = await dispatch(["show", "nonexistent-id"], makeBaseOpts(tmpDir));
		expect(result.exit).not.toBe(0);
...
describe("[unit] AC10 — discard", () => {
	...
	test("discard moves patch to .discarded/ and removes from list", async () => {
		// Propose
		const opts = makeBaseOpts(tmpDir, { stdin: makeStdin(VALID_DIFF) });
		const proposeResult = await dispatch(["propose", "docs/PRD.md", "--rationale=test"], opts);
		const draftId = proposeResult.stdout.trim();

		// Discard
		const discardResult = await dispatch(["discard", draftId], makeBaseOpts(tmpDir));
		...
		// In .discarded/
		const discardedPath = path.join(getDraftsDir(tmpDir), ".discarded", `${draftId}.patch`);
	...
	test("discard nonexistent id exits non-zero", async () => {
		const result = await dispatch(["discard", "no-such-draft"], makeBaseOpts(tmpDir));
		expect(result.exit).not.toBe(0);
	});
...
describe("[unit] docs apply — dry-run (no --i-approve)", () => {
	...
	test("without --i-approve exits 0 with preview and NOT YET APPLIED message", async () => {
		...
		const applyResult = await dispatch(["apply", draftId], makeBaseOpts(tmpDir));
...
describe("[unit] docs apply --i-approve — stub git", () => {
	...
	test("calls git apply --index and git commit; moves to .applied/", async () => {
		...
		// Propose
		const opts = makeBaseOpts(tmpDir, { stdin: makeStdin(VALID_DIFF), gitRunner });
		const proposeResult = await dispatch(["propose", "docs/PRD.md", "--rationale=add X section"], opts);
		const draftId = proposeResult.stdout.trim();
		...
		// Apply
		const applyResult = await dispatch(["apply", draftId, "--i-approve"], makeBaseOpts(tmpDir, { gitRunner }));
		expect(applyResult.exit).toBe(0);

		// git status called
		expect(gitCalls.some((c) => c.args[0] === "status")).toBe(true);
		// git apply called with --index
		expect(gitCalls.some((c) => c.args[0] === "apply" && c.args.includes("--index"))).toBe(true);
		// git commit called
		...
		// Moved to .applied/
		const appliedPath = path.join(getDraftsDir(tmpDir), ".applied", `${draftId}.patch`);
		expect(fs.existsSync(appliedPath)).toBe(true);

		// No longer in pending list
		const listResult = await dispatch(["list"], makeBaseOpts(tmpDir, { gitRunner }));
		expect(listResult.stdout).toContain("no pending drafts");
	});
	...
	test("dirty tree exits non-zero without calling git apply", async () => {
		...
		const gitRunner: DispatchOpts["gitRunner"] = (args, opts) => {
			gitCalls.push({ args, cwd: opts.cwd });
			if (args[0] === "status") return { stdout: "M docs/PRD.md\n", stderr: "", exit: 0 };
			return { stdout: "", stderr: "", exit: 0 };
		};
	...
	test("with open slice: commit includes Spec-Slice: trailer", async () => {
		...
		await dispatch(["apply", draftId, "--i-approve"], makeBaseOpts(tmpDir, { gitRunner }));
...
describe("[integration] AC9 — full propose → apply lifecycle", () => {
	...
	function makeIntegrationOpts(overrides: Partial<DispatchOpts> = {}): DispatchOpts {
		return {
			cwd: tmpDir,
			stdin: makeStdin(""),
			now: () => new Date("2026-04-24T14:02:11Z"),
	...
	test("AC9: propose then apply patches file and creates commit with correct trailers", async () => {
		...
		const original = fs.readFileSync(path.join(tmpDir, "docs", "PRD.md"), "utf8");
...
`;
		...
		// Apply
		const applyResult = await dispatch(["apply", draftId, "--i-approve"], makeIntegrationOpts());
		expect(applyResult.exit).toBe(0);
		...
		// Check file patched
		const patched = fs.readFileSync(path.join(tmpDir, "docs", "PRD.md"), "utf8");
...
`;
		...
		try {
			...
			const proposeResult = await dispatch(["propose", "docs/PRD.md", "--rationale=test dirty"], cleanOpts);
			const draftId = proposeResult.stdout.trim();
			expect(proposeResult.exit).toBe(0);
			...
			fs.copyFileSync(draftFile, path.join(targetDraftsDir, `${draftId}.patch`));
