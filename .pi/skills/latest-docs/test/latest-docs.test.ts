...
function mkEnv(registry?: Registry): TestEnv {
	...
	fs.mkdirSync(path.join(cwd, ".pi", "skills", "latest-docs"), { recursive: true });
	const r = registry ?? {
...
afterEach(() => {
	while (envs.length) {
		const e = envs.pop()!;
		try { fs.rmSync(e.tmpDir, { recursive: true, force: true }); } catch {}
	}
});
...
function mockFetch(responses: Record<string, { status?: number; body: string }>): FetchFn {
...
describe("scopeHtml", () => {
	test("extracts content of <main> tag", () => {
		const html = "<html><body><nav>skip</nav><main>keep this</main><footer>skip</footer></body></html>";
	...
	test("handles attributes on the tag", () => {
		expect(scopeHtml('<main id="x" class="y">body</main>', "main")).toBe("body");
	});
...
describe("fetch", () => {
	test("AC2: fetch writes Markdown with YAML frontmatter to dated cache path", async () => {
		const env = mkEnv();
		const fetchFn = mockFetch({
			"https://example.com/honcho-readme.md": { body: "# Honcho SDK\n\nuse it\n" },
		});
		const fakeNow = new Date("2026-04-24T10:00:00Z");
		...
		// Verify file on disk
		const file = path.join(env.cwd, ".pi", ".docs-cache", "@honcho-ai-sdk", "2026-04-24.md");
	...
	test("type=html runs htmlToMarkdown with selector scoping", async () => {
		const env = mkEnv();
		const fetchFn = mockFetch({
			"https://hono.example/docs": { body: "<html><nav>skip</nav><main>keep me</main></html>" },
		});
		const calls: string[] = [];
		const capturingHtmlToMd: HtmlToMarkdown = (h) => { calls.push(h); return `converted: ${h}`; };
		...
		const files = fs.readdirSync(path.join(env.cwd, ".pi", ".docs-cache", "hono"));
		expect(files.length).toBe(1);
		expect(fs.readFileSync(path.join(env.cwd, ".pi", ".docs-cache", "hono", files[0]!), "utf8"))
	...
	test("cache hit within TTL returns cached path without refetch", async () => {
		...
		const fakeNow = new Date("2026-04-24T10:00:00Z");
		await dispatch(["fetch", "@honcho-ai/sdk"], {
			cwd: env.cwd, env: {}, fetchFn, htmlToMarkdown: noopHtmlToMd, now: () => fakeNow,
		});
	...
	test("unknown lib refuses with register hint", async () => {
		...
		const result = await dispatch(["fetch", "not-registered"], {
			cwd: env.cwd, env: {}, fetchFn: mockFetch({}), htmlToMarkdown: noopHtmlToMd,
		});
	...
	test("non-2xx response surfaces HTTP error", async () => {
		const env = mkEnv();
		const fetchFn = mockFetch({
			"https://example.com/honcho-readme.md": { status: 500, body: "server error" },
		});
		const result = await dispatch(["fetch", "@honcho-ai/sdk"], {
			cwd: env.cwd, env: {}, fetchFn, htmlToMarkdown: noopHtmlToMd,
		});
...
describe("show", () => {
	...
	test("AC3: show prints stale warning when cache >TTL", async () => {
		...
		await dispatch(["fetch", "@honcho-ai/sdk"], {
			cwd: env.cwd, env: {}, fetchFn, htmlToMarkdown: noopHtmlToMd, now: () => fetched,
		});
		// Now query 10 days later
		const later = new Date("2026-04-11T10:00:00Z");
		const result = await dispatch(["show", "@honcho-ai/sdk"], {
			cwd: env.cwd, env: {}, fetchFn, htmlToMarkdown: noopHtmlToMd, now: () => later,
		});
		expect(result.exit).toBe(0);
	...
	test("AC4: show --section=X prints only that section", async () => {
		...
		const result = await dispatch(["show", "@honcho-ai/sdk", "--section=Installation"], {
			cwd: env.cwd, env: {}, fetchFn, htmlToMarkdown: noopHtmlToMd, now: () => fakeNow,
		});
		expect(result.exit).toBe(0);
...
describe("register", () => {
	test("AC5: register without --i-approve prints preview, no registry change", async () => {
		...
		const result = await dispatch(["register", "new-lib", "https://new.example/docs"], {
			cwd: env.cwd, env: {}, fetchFn: mockFetch({}), htmlToMarkdown: noopHtmlToMd,
		});
		expect(result.exit).toBe(0);
	...
	test("AC5: register with --i-approve adds to registry and appends audit log", async () => {
		...
		const result = await dispatch(
			["register", "new-lib", "https://new.example/docs", "--type=html", "--selector=article", "--ttl-days=14", "--i-approve"],
			{ cwd: env.cwd, env: {}, fetchFn: mockFetch({}), htmlToMarkdown: noopHtmlToMd, now: () => fakeNow },
		);
		expect(result.exit).toBe(0);
...
describe("turndown integration (real HTML→MD, no network)", () => {
	test("real turndown converts scoped HTML into expected Markdown", async () => {
		...
		await dispatch(["fetch", "hono"], {
			cwd: env.cwd, env: {}, fetchFn, htmlToMarkdown: realHtmlToMd,
		});

		const files = fs.readdirSync(path.join(env.cwd, ".pi", ".docs-cache", "hono"));
		expect(files.length).toBe(1);
		const content = fs.readFileSync(path.join(env.cwd, ".pi", ".docs-cache", "hono", files[0]!), "utf8");
