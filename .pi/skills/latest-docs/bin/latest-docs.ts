...
function registryPath(cwd: string): string {
	return path.join(cwd, ".pi", "skills", "latest-docs", "registry.json");
}
...
function isEntry(v: unknown): v is RegistryEntry {
	return !!v && typeof v === "object" && typeof (v as RegistryEntry).url === "string";
}
...
function writeCacheFile(
	...
	fs.writeFileSync(file, frontmatter + body + (body.endsWith("\n") ? "" : "\n"));
	return file;
}
...
async function cmdList(cwd: string, registry: Registry, now: Date): Promise<DispatchResult> {
	...
	for (const [lib, entry] of entries) {
		...
		if (recent) {
			...
			if (fm) {
				...
				lastFetched = age > ttl ? `${isoDate(new Date(fm.fetched_at))} (stale)` : isoDate(new Date(fm.fetched_at));
			}
		}
		lines.push(
...
async function cmdFetch(
	...
	if (!isEntry(entry)) {
		return { stdout: "", stderr: `not in registry: ${lib}\n\nRegister first: latest-docs register ${lib} <url> --i-approve`, exit: 1 };
	}
...
async function cmdShow(
	...
	if (!file) {
		const fetched = await cmdFetch(lib, { refresh: false }, cwd, registry, fetchFn, htmlToMarkdown, now);
		if (fetched.exit !== 0) return fetched;
		file = mostRecentCacheFile(cwd, lib);
		if (!file) {
			return { stdout: "", stderr: "fetch succeeded but cache file missing", exit: 2 };
		}
	}
	...
	if (opts.section) {
		...
		if (!section) {
			return { stdout: "", stderr: `section not found: ${opts.section}`, exit: 1 };
		}
...
async function cmdRegister(
	...
	const url = argv[1];
	if (!lib || !url) {
		return { stdout: "", stderr: "usage: latest-docs register <lib> <url> [--type=markdown|html] [--selector=CSS] [--ttl-days=N] [--i-approve]", exit: 1 };
	}
	...
	for (const arg of argv.slice(2)) {
		if (arg === "--i-approve") approve = true;
			...
			if (v !== "markdown" && v !== "html") {
				return { stdout: "", stderr: `invalid --type: ${v} (must be markdown|html)`, exit: 1 };
			}
			type = v;
		} else if (arg.startsWith("--selector=")) selector = arg.slice("--selector=".length);
		else if (arg.startsWith("--ttl-days=")) ttlDays = parseInt(arg.slice("--ttl-days=".length), 10);
	}

	const entry: RegistryEntry = { url, type, verified: false };
	if (selector) entry.selector = selector;
	if (typeof ttlDays === "number" && !Number.isNaN(ttlDays)) entry.ttl_days = ttlDays;
	...
	const prior = isEntry(registry[lib]) ? (registry[lib] as RegistryEntry) : null;
	registry[lib] = entry;
...
export async function dispatch(argv: string[], opts: DispatchOpts): Promise<DispatchResult> {
	...
	switch (command) {
		...
		case "fetch": {
			const lib = rest[0];
			if (!lib) return { stdout: "", stderr: "usage: latest-docs fetch <lib> [--refresh]", exit: 1 };
			const refresh = parseFlag(rest, "--refresh");
			return cmdFetch(lib, { refresh }, opts.cwd, registry, opts.fetchFn, opts.htmlToMarkdown, now);
		}

		case "show": {
			const lib = rest[0];
			if (!lib) return { stdout: "", stderr: "usage: latest-docs show <lib> [--section=X]", exit: 1 };
			const section = parseValue(rest, "--section=");
			return cmdShow(lib, { section }, opts.cwd, registry, opts.fetchFn, opts.htmlToMarkdown, now);
		}
